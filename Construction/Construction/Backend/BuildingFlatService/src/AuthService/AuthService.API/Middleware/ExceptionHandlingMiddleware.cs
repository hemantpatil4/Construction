...
public class ExceptionHandlingMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        if (statusCode >= 500)
            ...
            _logger.LogWarning("Handled exception: {ExceptionType} | Message: {Message} | TraceId: {TraceId} | Path: {Path}",
                exception.GetType().Name, exception.Message, traceId, context.Request.Path);
